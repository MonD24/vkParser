...
BLOCKED_KW = [
    ...
    # платное участие / взносы / скам-схемы
    "взнос", "вступительный взнос", "платное участие", "участие платное",
    "для участия оплат", "оплати и участвуй", "купи билет",
    "инвестиц", "инвестируй", "вложи деньги", "вложи средства",
    "займ", "кредит под", "финансовая пирамид",
    "партнёрская программа", "партнерская программа",
    "млм", "сетевой маркетинг", "network marketing",
    "доход от ", "заработок от ", "зарабатывай от ",
...
def _is_results_post(text: str) -> bool:
    """
    Комплексная проверка: пост объявляет итоги/победителя, а не проводит конкурс.
    Признаки итогового поста:
    - Ключевые слова итогов
    - Упоминание конкретного победителя через @[idXXX|Имя] или @упоминание
    - Прошедшее время («выиграл», «победил», «стал победителем»)
    - Паттерны «приз достался», «выиграл(а) X»
    """
    t = text.lower()

    # 1. Прямые слова-итоги
    RESULTS_KW = [
        "итоги розыгрыша", "итоги конкурса", "итоги гивэвея", "итоги giveaway",
        "победитель определён", "победитель определен",
        "результаты розыгрыша", "результаты конкурса",
        "достаётся ", "достается ", "достался ", "досталась ",
        "поздравляем победител",
        "объявляем победител",
        "определяем победител",
        "называем победител",
    ]
    if any(kw in t for kw in RESULTS_KW):
        log.debug("Пост отклонён: итоги (прямые ключевые слова)")
        return True

    # 2. Упоминание победителя через VK-тег [idXXX|Имя] рядом со словами победы
    WIN_CONTEXT = ["победител", "выиграл", "выиграла", "забирает", "забрал",
                   "достаётся", "достается", "приз получает", "приз уходит",
                   "поздравляем", "наш победитель", "наша победительница"]
    has_vk_mention = bool(re.search(r'\[id\d+\|[^\]]+\]', text))
    if has_vk_mention and any(kw in t for kw in WIN_CONTEXT):
        log.debug("Пост отклонён: итоги (упоминание победителя через [idXXX])")
        return True

    # 3. Прошедшее время победы без упоминания будущего конкурса
    PAST_WIN = [
        r"выиграл\w*\s+\w+",           # "выиграл Иван"
        r"победил\w*\s+\w+",           # "победила Анна"
        r"стал\w*\s+победител",         # "стала победителем"
        r"приз\s+(?:получил|забрал|ушёл|достался)",
    ]
    for pat in PAST_WIN:
        if re.search(pat, t):
            log.debug(f"Пост отклонён: итоги (паттерн прошедшего времени: {pat})")
            return True

    # 4. Рандомайзер/генератор победителей
    RANDOMIZER_KW = [
        "случайный выбор", "рандомайзер", "random.org",
        "генератор случайных", "выбрал рандомно",
    ]
    if any(kw in t for kw in RANDOMIZER_KW):
        log.debug("Пост отклонён: итоги (рандомайзер)")
        return True
...
def is_contest(text: str) -> bool:
    ...
    # Денежный приз (рубли/₽) — не интересует вне зависимости от наличия Steam
    if re.search(r"\d[\d\s.,]*(?:рублей|рубл|руб\.?|₽)", t):
        log.debug("Пост отклонён: денежный приз (рубли/₽)")
        return False
